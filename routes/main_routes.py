...
def _empty_summary():
    return {"total_spent":"0","top_category":"N/A","category_breakdown":{},"avg_daily":"0","num_transactions":0,"insights":[]}
...
def analytics():
    ...
    if not df.empty and "date" in df.columns:
        ...
        daily_totals = {k: int(v) for k,v in sorted(df.groupby(df["date"].dt.strftime("%Y-%m-%d"))["amount"].sum().to_dict().items())}
...
def transactions():
    ...
    if not df.empty:
        for _, row in df.sort_values("date", ascending=False).iterrows():
            ...
            txn_list.append({"date": str(row.get("date","")), "category": cat, "amount": float(row["amount"]), "color": p["color"], "bg": p["bg"], "icon": p["icon"]})
    categories = sorted(df["category"].dropna().unique().tolist()) if not df.empty else []
    return render_template("transactions.html", transactions=txn_list, categories=categories, summary=summary, now=_now(), active_page="transactions")
...
def budgets():
    ...
    if not df.empty:
        for cat, total in df.groupby("category")["amount"].sum().to_dict().items():
            ...
            if pct >= 100:
                sc, sl, bc = "over", "Over budget", "#ff6b6b"; over_count += 1
            elif pct >= 80:
                sc, sl, bc = "warn", "Near limit", "#f5a623"
            else:
                sc, sl, bc = "ok", "On track", "#00c9a7"
            budget_list.append({"category": cat, "spent": total, "limit": limit, "pct": min(pct,100), "status_class": sc, "status_label": sl, "bar_color": bc, "icon": p["icon"], "bg": p["bg"]})
    return render_template("budgets.html", budgets=budget_list, budget_over_count=over_count, summary=summary, now=_now(), active_page="budgets")
...
def schedule():
    ...
    return render_template("schedule.html", upcoming=upcoming, event_days=[e["day"] for e in upcoming], now=_now(), active_page="schedule")
